...
```text
install_transit.sh              # 中转机稳定入口，当前同步到 v6.91
install_landing.sh              # 落地机稳定入口，当前同步到 v6.91
install_amneziawg_dkms.sh       # AmneziaWG DKMS 独立入口，当前同步到 v6.91
install_transit_v6.91.sh        # v6.91 中转机版本快照
install_landing_v6.91.sh        # v6.91 落地机版本快照
install_amneziawg_dkms_v6.91.sh # v6.91 DKMS 版本快照
dd_debian.sh                    # Debian 12 DD 辅助命令生成器，默认不执行
verify_installation.sh          # 安装后验证脚本
versions.conf                   # 依赖和上游源码 ref 固定配置
zhubi.md                        # 主笔修复记录
docs/alternative-solutions.md   # 备用方案评判
docs/history.md                 # 旧版主笔记录归档
```

## 推荐系统基线

推荐使用 Debian 12 Bookworm minimal；`12.14` 是 Ghost-Proxy 已验证排障基线。当前 DD 辅助命令只固定 Debian 12，实际点版本由上游脚本和镜像决定。脚本不会自动 DD 或清盘，下面命令仅作为新机重装参考，执行前必须确认 VPS 商救援方式和 SSH 端口：
...
1. Mihomo 直导：推荐复制完整 raw YAML 到 Mihomo 配置/Profile。

```bash
cat /etc/landing-ghost/clash-meta-config.yaml
```

`clash-meta-subscription.txt` 仍保留为完整 Profile Base64 备用入口，只适合明确支持完整 Profile Base64 的客户端；不要把它粘贴到 Sub-Store。

2. Sub-Store 节点 Provider：推荐使用 provider-only 分离法。先在 ClashMeta/Mihomo 基础配置静态注入 `AWG-Tunnel`，再让 Sub-Store 只输出主轨/备轨。
Sub-Store 输入请使用 raw YAML，输出格式必须保持 Clash，不要转成 Surge/Stash/Sing-box，否则 `dialer-proxy` 可能被丢弃。

```bash
cat /etc/landing-ghost/mihomo-static-awg-proxy.yaml
cat /etc/landing-ghost/substore-provider-only.yaml
cat /etc/landing-ghost/clash-meta-substore-base.yaml
cat /etc/landing-ghost/substore-import-guide.txt
```

如果没有现成基础配置，直接复制 `clash-meta-substore-base.yaml`，把 `proxy-providers.ghost.url` 改成 Sub-Store 输出链接即可。ClashMeta 订阅侧引用 Sub-Store 输出链接时，建议隐藏底层隧道并只选择可见节点：
...
拿到 Sub-Store 输出链接后，可以做端到端验证：

```bash
curl -fsSL https://raw.githubusercontent.com/vpn3288/Ghost-Proxy/main/verify_installation.sh \
  | SUBSTORE_URL='你的 Sub-Store 输出链接' bash -s landing
```
...
预编译用户态兜底字段：`PREBUILT_AWG_GO_URL_x86_64`、`PREBUILT_AWG_GO_SHA256_x86_64`、`PREBUILT_AWG_TOOLS_URL_x86_64`、`PREBUILT_AWG_TOOLS_SHA256_x86_64`、`PREBUILT_AWG_GO_URL_arm64`、`PREBUILT_AWG_GO_SHA256_arm64`、`PREBUILT_AWG_TOOLS_URL_arm64`、`PREBUILT_AWG_TOOLS_SHA256_arm64`、`PREBUILT_AWG_GO_REF`、`PREBUILT_AWG_TOOLS_REF`、`PREBUILT_AWG_MANIFEST_URL_x86_64`、`PREBUILT_AWG_MANIFEST_URL_arm64`。当前默认使用 v6.85 Release 产物；下载、ref 校验或 SHA256 校验失败时，脚本会自动回退源码编译。

仓库包含 `.github/workflows/build-awg.yml`，可手动触发或在 tag 发布时构建 amd64/arm64 用户态产物，并发布 manifest JSON。更新预编译产物时，必须先发布 Release，再把真实 SHA256、manifest URL 和 manifest SHA256 回填到 `versions.conf` 和稳定入口脚本。
...
- 不提供 HTTP 订阅服务；`clash-meta-substore-base.yaml` 里的 `proxy-providers.type: http` 只是客户端引用外部 Sub-Store 输出链接。
- 默认入口只展示 Mihomo 直导和 Sub-Store 指南；其他 YAML/JSON/JS 文件仅保留给进阶、调试和历史兼容。
