...
```text
install_transit.sh              # 中转机稳定入口，当前同步到 v6.75
install_landing.sh              # 落地机稳定入口，当前同步到 v6.75
install_amneziawg_dkms.sh       # AmneziaWG DKMS 独立入口，当前同步到 v6.75
install_transit_v6.75.sh        # v6.75 中转机版本快照
install_landing_v6.75.sh        # v6.75 落地机版本快照
install_amneziawg_dkms_v6.75.sh # v6.75 DKMS 版本快照
...
- 不提供 HTTP 订阅服务；落地机安装完成后直接打印三条 `cat` 命令，用于完整显示 Mihomo Profile、Sub-Store provider、浏览器分流 JS 的 `GHOST_STATIC_PROXIES` 常量。
