...
DEFAULT_TARBALL_URL="https://github.com/amnezia-vpn/amneziawg-linux-kernel-module/archive/refs/heads/master.tar.gz"
...
install_packages() {
    ...
    retry_cmd "安装基础编译依赖" apt-get install -y -qq ca-certificates curl git dkms build-essential kmod make gcc libc6-dev pkg-config libmnl-dev \
        || die "基础编译依赖安装失败"
    ...
    if retry_cmd "安装当前内核头文件" apt-get install -y -qq "linux-headers-$(uname -r)"; then
        ...
        retry_cmd "安装架构通用头文件" apt-get install -y -qq "${ARCH_HEADER_PKG}" \
