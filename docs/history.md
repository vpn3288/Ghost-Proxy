...
本文件归档自 v6.91 清理前的 zhubi.md。当前审查请优先看仓库根目录 zhubi.md；旧记录仅用于追溯，不代表当前方案仍采用历史中的废弃做法。
# Ghost-Proxy 审核记录
...
- ✅ 长跑卫生复测：x86/ARM 落地配置备份均未超过最近 3 份，中转 sysctl/nft 备份均未超过最近 3 份；日志体积为 KB 级，根分区使用率 x86 10%、ARM 4%、中转 9%。
- ✅ 备份/日志复核后验证：两台落地机与中转机验证均 0 失败 0 警告，未发现反复重跑导致的磁盘或备份堆积问题。
- ✅ 防火墙矩阵复测：服务器间探测确认 80/443/8080/8443 均关闭；中转仅 SSH 与映射端口可用，落地直连 51820/8389 不可达。Windows 本地到 80/443 出现 TCP 握手但无响应的链路假阳性，服务器端无监听、无放行。
- ✅ 主动探测/GFW 类轻量复测：对中转 8389/8390 发送 HTTP/TLS/SOCKS/随机字节均无响应；对中转/落地 AWG UDP 发送随机包均无响应；短连接突发后全服务验证 0 失败 0 警告。
- ✅ 模拟人类交互安装：ARM 落地机不用 `AUTO_INSTALL`，模拟输错 IP、输错 AWG 端口、选错菜单项后继续输入正确值；安装成功并保持中转端口 51821/8390，落地与中转验证 0 失败 0 警告。
