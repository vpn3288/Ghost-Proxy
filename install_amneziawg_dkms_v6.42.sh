...
detect_platform() {
    ...
    if [[ "${ID:-}" != "debian" || "${VERSION_ID:-}" != "12" ]]; then
        ...
        warn "DD 示例会清空机器，请只在新机并确认商家救援能力后手动执行：bash reinstall.sh debian 12 --ssh-port 22"
    fi
...
install_packages() {
    ...
    if [[ -n "${DKMS_VERSION:-}" ]]; then
        ...
        base_packages+=("dkms=${DKMS_VERSION}")
    else
        base_packages+=(dkms)
    fi
    if [[ -n "${GCC_VERSION:-}" ]]; then
        info "使用固定 GCC 主版本: ${GCC_VERSION}"
        base_packages+=("gcc-${GCC_VERSION}")
    else
        base_packages+=(gcc)
    fi
    ...
    retry_cmd "安装基础编译依赖" apt-get install -y -qq "${base_packages[@]}" \
        || die "基础编译依赖安装失败"
    if [[ -n "${GCC_VERSION:-}" && -x "/usr/bin/gcc-${GCC_VERSION}" ]]; then
...
print_result() {
    ...
    echo "  apt-mark hold linux-image-amd64 linux-headers-amd64"
}
