...
print_dd_baseline_hint() {
    ...
    warn "DD/网络重装会清空机器；仅限新机并确认救援能力后手动执行。统一入口："
...
EOF
    cat > /etc/systemd/system/ghost-awg-dkms-check.timer <<'EOF'
[Unit]
Description=AmneziaWG DKMS hourly health check
...
EOF
    ...
    systemctl enable --now ghost-awg-dkms-check.timer >/dev/null 2>&1 || true
