...
cleanup() {
    ...
    if [[ "${BUILD_SWAP_CREATED}" -eq 1 && "${KEEP_BUILD_SWAP:-0}" != "1" ]]; then
        warn "临时 swap 将在 1 小时后自动清理；如需保留请设置 KEEP_BUILD_SWAP=1"
        nohup bash -c "sleep 3600; swapoff '${BUILD_SWAP_FILE}' 2>/dev/null || true; rm -f '${BUILD_SWAP_FILE}' 2>/dev/null || true" >/dev/null 2>&1 &
    fi
}
trap cleanup EXIT
